...
if ((!isset($_POST['filter_type']) || $_POST['filter_type']=='')  && (!isset($_POST['filter_nation']) || $_POST['filter_nation']=='') && (!isset($_POST['filter_year']) || $_POST['filter_year']=='')) {
    ...
    elseif ($_GET['type'] == 'single-movie') {
         $year = $_GET['year'];
         $sql_filter = "select * from `film` where `type_id` = 1 and `year` = '$year'";
    }
    elseif ($_GET['type'] == 'series-movie') {
         $year = $_GET['year'];
         $sql_filter = "select * from `film` where `type_id` = 2 and `year` = '$year'";
    }
    else {
         $year = $_GET['year'];
         $sql_filter = "select * from `film` where `type_id` = 3 and `year` = '$year'";
    }
}
else {
    if ($_GET['type'] == 'category') {
...
//            }
    ...
    elseif ($_GET['type'] == 'single-movie') {
        ...
        if ($_POST['filter_type'] != '' && $_POST['filter_nation'] == '' && $_POST['filter_year'] == '') {
             $filter_type = $_POST['filter_type'];
            if ($filter_type == 'filter_id') {
                  $sql_filter = "select * from `film` where `type_id` = 1 order by `id` desc limit 32";
            }
            elseif ($filter_type == 'filter_view') {
                  $sql_filter = "select * from `film` where `type_id` = 1 order by `num_view` desc limit 32";
            }
            elseif ($filter_type == 'filter_name_desc') {
                  $sql_filter = "select * from `film` where `type_id` = 1 order by `name` desc limit 32";
            }
            elseif ($filter_type == 'filter_name_asc') {
                  $sql_filter = "select * from `film` where `type_id` = 1 order by `name` asc limit 32";
            }
            elseif ($filter_type == 'filter_lenght') {
                  $sql_filter = "select * from `film` where `type_id` = 1 order by `duration` desc limit 32";
            }
        }
        // type = 1,nation = 0, year = 0
        elseif ($_POST['filter_type'] != '' && $_POST['filter_nation'] != '' && $_POST['filter_year'] == '') {
            ...
            $filter_nation = $_POST['filter_nation'];
            if ($filter_type == 'filter_id') {
                  $sql_filter = "select * from `film` where `type_id` = 1 and `nation_id` = '$filter_nation' order by `id` desc limit 32";
            }
            elseif ($filter_type == 'filter_view') {
                  $sql_filter = "select * from `film` where `type_id` = 1 and `nation_id` = '$filter_nation' order by `num_view` desc limit 32";
            }
            elseif ($filter_type == 'filter_name_desc') {
                  $sql_filter = "select * from `film` where `type_id` = 1 and `nation_id` = '$filter_nation' order by `name` desc limit 32";
            }
            elseif ($filter_type == 'filter_name_asc') {
                  $sql_filter = "select * from `film` where `type_id` = 1 and `nation_id` = '$filter_nation' order by `name` asc limit 32";
            }
            elseif ($filter_type == 'filter_lenght') {
                  $sql_filter = "select * from `film` where `type_id` = 1 and `nation_id` = '$filter_nation' order by `duration` desc limit 32";
            }
        }
        // type = 1,nation = 1, year = 0
        elseif ($_POST['filter_type'] == '' && $_POST['filter_nation'] != '' && $_POST['filter_year'] == '') {
            $filter_nation = $_POST['filter_nation'];
             $sql_filter = "select * from `film` where `type_id` = 1 and `nation_id` = '$filter_nation' limit 32";
        }
        // type = 0,nation = 1, year = 0
    }
    elseif ($_GET['type'] == 'series-movie') {
        ...
        if ($_POST['filter_type'] != '' && $_POST['filter_nation'] == '' && $_POST['filter_year'] == '') {
             $filter_type = $_POST['filter_type'];
            if ($filter_type == 'filter_id') {
                  $sql_filter = "select * from `film` where `type_id` = 2 order by `id` desc limit 32";
            }
            elseif ($filter_type == 'filter_view') {
                  $sql_filter = "select * from `film` where `type_id` = 2 order by `num_view` desc limit 32";
            }
            elseif ($filter_type == 'filter_name_desc') {
                  $sql_filter = "select * from `film` where `type_id` = 2 order by `name` desc limit 32";
            }
            elseif ($filter_type == 'filter_name_asc') {
                  $sql_filter = "select * from `film` where `type_id` = 2 order by `name` asc limit 32";
            }
            elseif ($filter_type == 'filter_lenght') {
                  $sql_filter = "select * from `film` where `type_id` = 2 order by `duration` desc limit 32";
            }
        }
        // type = 1,nation = 0, year = 0
        elseif ($_POST['filter_type'] != '' && $_POST['filter_nation'] != '' && $_POST['filter_year'] == '') {
            ...
            $filter_nation = $_POST['filter_nation'];
            if ($filter_type == 'filter_id') {
                  $sql_filter = "select * from `film` where `type_id` = 2 and `nation_id` = '$filter_nation' order by `id` desc limit 32";
            }
            elseif ($filter_type == 'filter_view') {
                  $sql_filter = "select * from `film` where `type_id` = 2 and `nation_id` = '$filter_nation' order by `num_view` desc limit 32";
            }
            elseif ($filter_type == 'filter_name_desc') {
                  $sql_filter = "select * from `film` where `type_id` = 2 and `nation_id` = '$filter_nation' order by `name` desc limit 32";
            }
            elseif ($filter_type == 'filter_name_asc') {
                  $sql_filter = "select * from `film` where `type_id` = 2 and `nation_id` = '$filter_nation' order by `name` asc limit 32";
            }
            elseif ($filter_type == 'filter_lenght') {
                  $sql_filter = "select * from `film` where `type_id` = 2 and `nation_id` = '$filter_nation' order by `duration` desc limit 32";
            }
        }
        // type = 1,nation = 1, year = 0
        elseif ($_POST['filter_type'] == '' && $_POST['filter_nation'] != '' && $_POST['filter_year'] == '') {
            $filter_nation = $_POST['filter_nation'];
             $sql_filter = "select * from `film` where `type_id` = 2 and `nation_id` = '$filter_nation' limit 32";
        }
        // type = 0,nation = 1, year = 0
    }
    else {
        ...
        if ($_POST['filter_type'] != '' && $_POST['filter_nation'] == '' && $_POST['filter_year'] == '') {
             $filter_type = $_POST['filter_type'];
            if ($filter_type == 'filter_id') {
                  $sql_filter = "select * from `film` where `type_id` = 3 order by `id` desc limit 32";
            }
            elseif ($filter_type == 'filter_view') {
                  $sql_filter = "select * from `film` where `type_id` = 3 order by `num_view` desc limit 32";
            }
            elseif ($filter_type == 'filter_name_desc') {
                  $sql_filter = "select * from `film` where `type_id` = 3 order by `name` desc limit 32";
            }
            elseif ($filter_type == 'filter_name_asc') {
                  $sql_filter = "select * from `film` where `type_id` = 3 order by `name` asc limit 32";
            }
            elseif ($filter_type == 'filter_lenght') {
                  $sql_filter = "select * from `film` where `type_id` = 3 order by `duration` desc limit 32";
            }
        }
        // type = 1,nation = 0, year = 0
        elseif ($_POST['filter_type'] != '' && $_POST['filter_nation'] != '' && $_POST['filter_year'] == '') {
            ...
            $filter_nation = $_POST['filter_nation'];
            if ($filter_type == 'filter_id') {
                  $sql_filter = "select * from `film` where `type_id` = 3 and `nation_id` = '$filter_nation' order by `id` desc limit 32";
            }
            elseif ($filter_type == 'filter_view') {
                  $sql_filter = "select * from `film` where `type_id` = 3 and `nation_id` = '$filter_nation' order by `num_view` desc limit 32";
            }
            elseif ($filter_type == 'filter_name_desc') {
                  $sql_filter = "select * from `film` where `type_id` = 3 and `nation_id` = '$filter_nation' order by `name` desc limit 32";
            }
            elseif ($filter_type == 'filter_name_asc') {
                  $sql_filter = "select * from `film` where `type_id` = 3 and `nation_id` = '$filter_nation' order by `name` asc limit 32";
            }
            elseif ($filter_type == 'filter_lenght') {
                  $sql_filter = "select * from `film` where `type_id` = 3 and `nation_id` = '$filter_nation' order by `duration` desc limit 32";
            }
        }
        // type = 1,nation = 1, year = 0
        elseif ($_POST['filter_type'] == '' && $_POST['filter_nation'] != '' && $_POST['filter_year'] == '') {
            $filter_nation = $_POST['filter_nation'];
             $sql_filter = "select * from `film` where `type_id` = 3 and `nation_id` = '$filter_nation' limit 32";
        }
        // type = 0,nation = 1, year = 0
    }
}
