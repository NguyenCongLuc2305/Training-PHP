        ...
        <div class="col">
            ...
            <div class="blockbody" id="phim-bo-hay">
                <ul class="list tab phim-bo-moi">
                    <?php
                    $sql = 'select * from `film` where `type_id` = 1 order by `id` DESC limit 8';
                ...
                <ul class="list tab phim-bo-full hide">
                    <?php
                    $sql = 'select * from `film` where `type_id` = 2 order by `id` DESC limit 8';
                    $query = mysqli_query($link, $sql);
                    while($r=mysqli_fetch_assoc($query)){
                        ?>
                        <li><a href="?mod=detail&film_id=<?php echo $r['id'] ?>" title="<?php echo $r['name'] ?>"><?php echo $r['name'] ?></a><span><?php echo $r['status'] ?></span></li>
        ...
        <div class="blockbody" id="list-movie-update">
                ...
                <ul class="list-film">
                    ...
                    while($r=mysqli_fetch_assoc($query)){
                        ...
                        <li data-liked="852" data-views="49,875">
                            <div class="inner"><a href="?mod=detail&film_id=<?php echo $r['id'] ?>" title="<?php echo $r['name'] ?>"><img src="admin/uploads/images/<?php echo $r['image'] ?>" alt="Nhật Ký Trả Thù 2"></a>
                                <div class="info">
            ...
            <div class="tab phim-le hide">
                <ul class="list-film tab phim-le">
                    <?php
                    $sql = 'select * from `film` where `type_id` = 1 order by `id` DESC limit 12';
                    $query = mysqli_query($link, $sql);
                    while($r=mysqli_fetch_assoc($query)){
                        ?>
                        <li data-liked="3,987" data-views="94,650">
                            <div class="inner"><a href="?mod=detail&film_id=<?php echo $r['id'] ?>" title="<?php echo $r['name'] ?>"><img src="admin/uploads/images/<?php echo $r['image'] ?>" alt="<?php echo $r['name'] ?>"></a>
                                <div class="info">
                                    <div class="name"><a href="?mod=detail&film_id=<?php echo $r['id'] ?>" title="<?php echo $r['name'] ?>"><?php echo $r['name'] ?></a></div>
                                </div>
                                <div class="status"><?php echo $r['status'] ?></div>
            ...
            <div class="tab phim-bo hide">
                <ul class="list-film">
                    <?php
                    $sql = 'select * from `film` where `type_id` = 2 order by `id` DESC limit 12';
                    $query = mysqli_query($link, $sql);
                    while($r=mysqli_fetch_assoc($query)){
                        ?>
                        <li data-liked="3,987" data-views="94,650">
