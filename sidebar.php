    ...
    <div class="block" id="chart">
        ...
        <div class="blockbody" id="topview">
            <ul class="tab topviewday">
                <?php
                $sql = 'select * from `film` where `type_id` = 1 order by `id` DESC limit 10';
            ...
            <ul class="tab topviewweek hide">
                <?php
                $sql = 'select * from `film` where `type_id` = 2 order by `id` DESC limit 10';
                $query = mysqli_query($link, $sql);
                while($r=mysqli_fetch_assoc($query)){
                    ?>
                    <li><span class="st top">1</span>
            ...
            <ul class="tab topviewmonth hide">
                <?php
                $sql = 'select * from `film` where `type_id` = 3 order by `id` DESC limit 10';
                $query = mysqli_query($link, $sql);
                while($r=mysqli_fetch_assoc($query)){
                    ?>
                    <li><span class="st top">1</span>
